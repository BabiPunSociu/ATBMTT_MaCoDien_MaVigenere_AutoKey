...
# ma Vigenere _ khoa tu dong (auto key)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if luaChon == 0:    # Thoat
            ...
            if len(khoa) < len(ban_ro):
                khoa += ban_ro[:len(ban_ro)-len(khoa)]
            # Ma hoa
            ban_ma = ''
            for i in range(len(ban_ro)):
                ban_ma += ma_hoa(ban_ro[i], khoa[i], bangchucai) 
            ...
            for i in range(len(ban_ma)):
                # Tham ma bang key da biet
                if i < len(khoa):   
                    ban_ro += giai_ma(ban_ma[i], khoa[i], bangchucai) 
                # Tham ma bang phan dau cua ban ro (phan duoc noi vao key)
                else:
                    ban_ro += giai_ma(ban_ma[i], ban_ro[i-len(khoa)], bangchucai) 
